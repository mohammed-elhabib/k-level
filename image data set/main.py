...
def plt_3d(level_data, data):
    for level_idx, clusters_at_level in enumerate(level_data):
        ...
        for cluster in clusters_at_level.get_clusters():
             #points = data[cluster.get_values()]
             ax.scatter(cluster.get_values()[0], cluster.get_values()[1], cluster.get_values()[2])  # Corrected indexing here
        ...
        plt.savefig(f"Level {level_idx + 1}")
        plt.tight_layout()
        plt.show()  #

def plt_2d(level_data, data):
    for level_idx, clusters_at_level in enumerate(level_data):
        ...
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.save(f"2d/Level {level_idx + 1}")
...
data = read_image_and_preprocess(image_path)
print(len(data))
# Example usage
...
# Evaluate clustering
evaluate_clustering(data, level_data)
